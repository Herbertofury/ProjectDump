...
if len(sys.argv) != 2:
    raise SystemExit('usage: apply_perf_instrumentation_source_pairs.py <merged-upstream-root>')
...
'''                if (!entity.isAlive()) continue;
                ...
                if (entity.getTags().contains("harimt_local_noise")) ++localNoiseTagged;
                ++totalEntities;''',
...
'''        System.out.println(
                "HMT_PERF_STATS mspt=" + mspt
                        ...
                        + " localNoiseTagged=" + localNoiseTagged);
        source.sendSuccess(() -> message, false);
    }
...
for path, markers in (
    (stats, ('HMT_PERF_STATS', 'perfTagged=', 'noiseTagged=', 'localNoiseTagged=')),
...
print('HariMultiThread Ultimate symmetric source-pairs performance telemetry applied')
