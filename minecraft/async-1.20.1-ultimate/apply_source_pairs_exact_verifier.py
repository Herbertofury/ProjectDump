...
if len(sys.argv) != 2:
    raise SystemExit('usage: apply_source_pairs_exact_verifier.py <source-pairs-candidate-root>')
...
def once(old,new,label):
    global s
    n=s.count(old)
    if n!=1: raise SystemExit(f'{label}: expected 1 marker, found {n}')
...
insert='''
    ...
    private static void runSourcePairVerification(CommandSourceStack source) {
        ...
        Optional<List<GpuCollisionDispatcher.CollisionPair>> result =
                d.computeGpuOnlyForSources(population, sources);
        if (result.isEmpty()) {
        ...
        Set<Long> missing=new HashSet<>(exact); missing.removeAll(gpu);
        Set<Long> extras=new HashSet<>(gpu); extras.removeAll(exact);
        if (!missing.isEmpty()) {
...
'''
if s.count(marker)!=1: raise SystemExit('exactPairs insertion marker drift')
s=s.replace(marker,insert+marker,1)
for m in ('Commands.literal("source-test")','computeGpuOnlyForSources(population, sources)','HMT_SOURCE_PAIRS_VERIFY PASS','expectedPopulation=768 expectedSources=256'):
    if s.count(m)!=1: raise SystemExit(f'verifier invariant failed: {m} count={s.count(m)}')
p.write_text(s,encoding='utf-8')
print('source-pairs exact verifier instrumentation applied')
