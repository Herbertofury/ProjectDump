...
# Upstream C2ME declares Commons Compress with transitive=false. Shade it into a non-modular
# JarJar payload so Forge's module resolver does not require unused transitive modules such as
# commons-codec while the TAR shader-cache classes remain available.
commons_compress_runtime = merge_jars([
    libs_dir / 'commons-compress-1.28.0.jar',
], 'commons-compress-c2me-runtime-1.28.0.jar')
...
runtime_paths = [
    ...
    add_outer('c2me.runtime', 'commons-compress', '1.28.0', commons_compress_runtime, 'commons-compress-c2me-runtime-1.28.0.jar'),
]
...
entries['META-INF/c2me-opencl-radium-compat.txt'] = (
    ...
    'Runtime hardening: LWJGL core/OpenCL Windows payload, Zstd Windows payload, Caffeine 3.2.1, and non-modular Commons Compress 1.28.0 payload vendored in outer JarJar.\n'
...
with zipfile.ZipFile(out) as z:
    ...
    with zipfile.ZipFile(io.BytesIO(z.read(runtime_paths[3]))) as rt:
        commons_names = set(rt.namelist())
        assert 'org/apache/commons/compress/archivers/ArchiveEntry.class' in commons_names
        assert not any(name.endswith('module-info.class') for name in commons_names)
